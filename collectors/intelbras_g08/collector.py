...
DOCUMENTED_COMMANDS = {
    ...
    "ont_brief": G08_ONT_BRIEF_COMMAND,
...
class IntelbrasG08Collector:
    ...
    def collect_from_texts(
        ...
        *,
        ...
        if mac_table_text:
            ...
            completeness = "complete" if onus else "none"
    ...
    def collect_via_transport(
        self, transport: Transport, *, enabled_collectors: Optional[frozenset[str]] = None
    ) -> CollectorResult:
        enabled = enabled_collectors or frozenset({"ont_mac_table"})
        commands = [
            ("ont_mac_table", G08_MAC_TABLE_COMMAND),
            ("ont_brief", G08_ONT_BRIEF_COMMAND),
        ]
        ...
        for key, command in commands:
            ...
            if not response.ok or not (response.stdout or "").strip():
                failures += 1
                continue
            outputs[key] = response.stdout
        if not outputs:
            return self._failed("no enabled G08 command returned usable output", G08_MAC_TABLE_COMMAND)
        parsed = self.collect_from_texts(
            mac_table_text=outputs.get("ont_mac_table", ""),
            ont_brief_text=outputs.get("ont_brief", ""),
            mac_command=G08_MAC_TABLE_COMMAND,
        )
        completeness = parsed.meta.get("completeness", "none")
        if failures:
            completeness = "partial" if (parsed.olt_macs or parsed.onus) else "none"
        status = "ok" if completeness == "complete" else ("partial" if completeness == "partial" else "error")
        parsed.meta.update(
            {
                "mode": "transport", "status": status,
                "commands_ok": len(outputs), "commands_failed": failures,
                "collector_version": self.collector_version,
                "command": ",".join(DOCUMENTED_COMMANDS[key] for key in outputs),
                "family": G08_FAMILY,
            }
        )
        return parsed
    ...
    def collect_live(self, *, enabled_collectors: Optional[frozenset[str]] = None) -> CollectorResult:
        if enabled_collectors is not None and not {"ont_mac_table", "ont_brief"}.intersection(enabled_collectors):
            return CollectorResult(
                meta={
                    "mode": "live",
                    "status": "skipped",
        ...
        if self._transport is not None:
            return self.collect_via_transport(self._transport, enabled_collectors=enabled_collectors)
